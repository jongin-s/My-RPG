...
class EnemyObject(BaseObject):
    def __init__(self, spr, coord, kinds, game, types):
        ...
        self.hpm = 0
        self.hp = 0
        self.game.enemies.append(self)  # 적 객체를 게임의 적 리스트에 추가
    ...
    def events(self):
        ...
        if not self.destroy:
            ...
            if self.types == 'snake':  # 뱀일 경우
                ...
                if self.vspeed >= 0:
                    if self.frameTimer >= self.frameSpeed:
                        self.frameTimer = 0
                        self.spr_index = 1 if self.spr_index == 0 else 0
                else:
                    self.spr_index = 2
        ...
        self.game.check_enemies_status()  # 모든 적 객체가 파괴되었는지 확인
...
def createMapData():
    ...
    ground_size_count = 0
    ground_height = ground_baseheight + random.randrange(-2, 1)           # 바닥 높이
